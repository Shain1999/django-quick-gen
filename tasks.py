...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-quick-gen runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-quick-gen.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_quick_gen")
...
def lint(c):
    ...
    """
    c.run("flake8 django-quick-gen tests")
...
def release(c, bumpsize=''):
    ...
    import django_quick_gen
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_quick_gen.__version__))
    c.run("git push --tags")
